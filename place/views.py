...
class PlaceView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"place": serializer.data, "total-page": total_items},
...
class PlaceDetailView(APIView):
    ...
    def post(self, request, place_id):
        ...
        if user in place.bookmark.all():
            ...
            return Response(
                {"message": "북마크 취소", "book_count": bookmark_count}, status.HTTP_200_OK
            )
        else:
            place.bookmark.add(user)
            bookmark_count = place.bookmark.count()
            return Response(
                {"message": "북마크", "book_count": bookmark_count}, status.HTTP_200_OK
            )
...
class PlaceLikeView(APIView):
    ...
    def post(self, request, place_id):
        ...
        if request.user in place.like.all():
            ...
            return Response(
                {"message": "좋아요 취소", "like_count": like_count}, status.HTTP_200_OK
            )
        else:
            place.like.add(request.user)
            like_count = place.like.count()
            return Response(
                {"message": "좋아요", "like_count": like_count}, status.HTTP_200_OK
            )
...
class PlaceCommentDetailView(APIView):
    ...
    def post(self, request, place_id, place_comment_id):
        ...
        if serializer.is_valid():
            ...
            if upside_comment.deep >= 1:
                return Response(
                    {"message": "대댓글에 댓글을 작성할 수 없습니다."}, status.HTTP_403_FORBIDDEN
                )
            # (상위 댓글이 없는 경우) 대댓글
            else:
                serializer.save(
                    user=request.user,
                    place=place,
                    main_comment=upside_comment,
                    deep=upside_comment.deep + 1,
                )
                return Response(serializer.data, status.HTTP_200_OK)
        else:
            return Response(serializer.errors, status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, place_id, place_comment_id):
        ...
        if request.user == comment.user:
            serializer = PlaceCreateCommentSerializer(comment, data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status.HTTP_200_OK)
            else:
...
class PlaceTitleSearchView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            ...
            .get_queryset()
            .annotate(
                comment_count=Count("place_comment_place"),
                likes_count=Count("like"),
                books_count=Count("bookmark"),
            )
        )
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated:
            try:
                ...
                if (
                    current_region1 is not None
                    and current_region2 is not None
                    and search_query
                ):
                    queryset = Place.objects.filter(
                        Q(address__contains=current_region1)
                        & Q(address__contains=current_region2)
                    )
                    ...
                    if not queryset:
                        queryset = Place.objects.filter(
                            address__startswith=current_region1
                        )
                        queryset = queryset.filter(category__icontains=search_query)
